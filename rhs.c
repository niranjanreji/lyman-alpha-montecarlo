...
/*  for (nv = NVAR; nv--;  ) vc[nv] = vh[i][nv]; */
      ...
      // =====================================
      // ADD RADIATIVE PRESSURE SOURCE TERM
      // Convert local index i to global index for force array access
      // =====================================

      if (g_lya_radForce != NULL) {
        int i_glob = i + grid[IDIR].beg - grid[IDIR].nghost;
        if (i_glob >= 0 && i_glob < g_lya_radForceSize) {
          #if COUPLE_LYA_RT == TRUE
            rhs[i][iMR] += dt * g_lya_radForce[i_glob];
          #endif
          #if IDEAL_EOS
            #if COUPLE_LYA_RT == TRUE
              #if ENERGY_DEPOSIT == FDV
                rhs[i][ENG] += dt * g_lya_radForce[i_glob] * vc[iVR];
              #else 
                rhs[i][ENG] += dt * g_lya_radEnergy[i_glob];
              #endif
            #endif
          #endif
        }
